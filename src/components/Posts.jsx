...
function Posts() {
  ...
  const [likes, setLikes] = useState(
    blogPosts.map((posts) => posts.map((post) => post.likes))
  );

  function handleLikeClick(index1, index2) {
    const updatedLikes = [...likes];
    updatedLikes[index1][index2] += 1;
    setLikes(updatedLikes);
  }

  function handleDislikeClick(index1, index2) {
    if (likes[index1][index2] > 0) {
      const updatedLikes = [...likes];
      updatedLikes[index1][index2] -= 1;
      setLikes(updatedLikes);
    }
              ...
              <div className="post-item" key={post.id}>
                <div className="post-header">
                  ...
                  <div className="post-social-media-stats">
                    ...
                    <span className="post-likes">{likes[index1][index2]}</span>
                  </div>
                </div>
                ...
                <div className="post-actions">
                  <button
                    ...
                    onClick={() => handleLikeClick(index1, index2)}>
                    Like
                  </button>
                  <button
                    className="dislike-button"
                    onClick={() => handleDislikeClick(index1, index2)}>
                    Dislike
                  </button>
                </div>
              </div>
